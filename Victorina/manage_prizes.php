...
// Обработка добавления нового подарка
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_prize'])) {
    $prize_name = $mysqli->real_escape_string($_POST['prize_name']);
    $quantity = (int)$_POST['quantity'];

    $mysqli->query("INSERT INTO prizes (prize_name, quantity) VALUES ('$prize_name', $quantity)");
    $message = "Подарок добавлен!";
}

// Обработка удаления подарка
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_prize'])) {
    $prize_id = (int)$_POST['prize_id'];

    $mysqli->query("DELETE FROM prizes WHERE id = $prize_id");
    $message = "Подарок удален!";
}

// Получение списка всех подарков
$prizes_result = $mysqli->query("SELECT * FROM prizes");
$prizes = [];
...
<head>
    ...
    <!-- SEO Meta Tags -->
    <meta name="description" content="Страница загрузки подарков">
	...
	<meta property="og:title" content="Загрузка подарков викторины"/> <!-- title shown in the actual shared post -->
	<meta property="og:description" content="Страница загрузки подарков" /> <!-- description shown in the actual shared post -->
	<meta property="og:image" content="images/ogimage.jpg" /> <!-- image link, make sure it's jpg -->
	<meta property="og:type" content="article" />

    <!-- Website Title -->
    <title>Загрузка подарков</title>
...
<!-- Форма для добавления нового подарка -->
<form method="post">
    <div class="form-group">
        <label for="prize_name">Название подарка:</label>
    ...
    <button type="submit" name="add_prize">Добавить подарок</button>
</form>
<div style="height:70px;">
</div>
<!-- Таблица с существующими подарками -->
            ...
            <tr style="color:#fff;">
                ...
                <td>
                    <form method="post" style="display:inline;">
                        ...
                        <button type="submit" name="delete_prize" style="background-color: red; color: white;">Удалить</button>
                    </form>
                </td>
            </tr>
        <?php endforeach; ?>
	...
	<div style="background-color:#222;height:70px;">
		...
		<div style="float:right;margin-right:30px;">
			...
			<div style="margin: 10px 0 0 30px;float:right;">
				<form method="post" action="admin.php" style="float:left;">
